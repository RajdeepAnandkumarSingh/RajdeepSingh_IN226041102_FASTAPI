...
app = FastAPI(title="FastAPI Assignment 2")
...
products = [
    ...
    {"id": 3, "name": "Bluetooth Speaker", "price": 2999, "category": "Electronics", "in_stock": False},
...
def place_bulk_order(order: BulkOrder):
    confirmed, failed, total_bill = [], [], 0
    for item in order.items:
        product = next((p for p in products if p["id"] == item.product_id), None)
        if not product or not product["in_stock"]:
            failed.append({"product_id": item.product_id, "reason": "Unavailable"})
        else:
            subtotal = product["price"] * item.quantity
            total_bill += subtotal
            confirmed.append({"product": product["name"], "qty": item.quantity, "subtotal": subtotal})
    return {"company": order.company_name, "confirmed": confirmed, "failed": failed, "grand_total": total_bill}